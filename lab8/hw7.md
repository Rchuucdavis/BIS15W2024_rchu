...
```r
amniota <- replace_with_na_all(amniota, condition = ~.x == -999)
```
...
The NAs in these columns could mean that an organism was not recorded having that trait, however it could be possible for them to have it. 
